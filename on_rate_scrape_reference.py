...
Usage:
    python collect_interbank_incremental.py           # Daily update
    python collect_interbank_incremental.py --check   # Check status
"""
import argparse
import os
import pandas as pd
...
# Import from scrape_sbv_interbank
import sys
sys.path.insert(0, '.')
...
def main():
    ...
    if not csv_file:
        if args.init:
            ...
            print("  python collect_interbank_incremental.py --init interbank_rates_complete.csv")
            return
    ...
    if not new_rates:
        print("  ❌ Failed to fetch new rates")
        return
