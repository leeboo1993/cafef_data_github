#!/bin/bash
set -e
echo "🚀 Running Unified CafeF Data Downloader..."
python cafef_data_downloader.py

echo "🚀 Running gold price updater..."
python gold_price_scraper.py

echo "🚀 Running deposit rate scraper (last 3 days)..."
START_DATE=$(date -v-3d +%Y-%m-%d 2>/dev/null || date -d "3 days ago" +%Y-%m-%d)
python vietnamnet_interest_rate.py --start_date=$START_DATE

echo "🚀 Running VCB FX scraper (T-5 to today)..."
